...
# Updated for Render deployment - v12
app = Flask(__name__)
...
@app.route('/version')
def version():
    return jsonify({"status": "updated", "version": "v12"})

@app.route('/test')
def test():
    return jsonify({"message": "This is a test endpoint", "timestamp": datetime.now().isoformat()})

@app.route('/my-wins', methods=['GET', 'POST'])
@jwt_required()
def my_wins():
    print("DEBUG: ===== MY-WINS ENDPOINT CALLED =====")
    ...
    print(f"DEBUG: My-wins endpoint called with method {request.method}, user_id: {user_id}")
...
@app.route('/wins-test', methods=['GET', 'POST'])
@jwt_required()
def wins_test():
    print("DEBUG: ===== WINS-TEST ENDPOINT CALLED =====")
    print(f"DEBUG: Request method: {request.method}")
    print(f"DEBUG: Request headers: {dict(request.headers)}")
    print(f"DEBUG: Request data: {request.get_data()}")
    ...
    print(f"DEBUG: Wins-test endpoint called with method {request.method}, user_id: {user_id}")
...
def wins():
    ...
    # Temporarily hardcode user_id to get production working
    user_id = 1
